...
static bool rp2040_swd_init_debug(uint8_t core)
{
    ...
    picoprobe_info("rp2040_swd_init_debug: start core=%u\n", core);
    swd_init();
    picoprobe_info("rp2040_swd_init_debug: swd_init done\n");
    swd_from_dormant();
    picoprobe_info("rp2040_swd_init_debug: swd_from_dormant done\n");

    /* quick diagnostic: read DP IDCODE and CTRL_STAT to verify SWD comms */
    {
        uint32_t dp_id = 0;
        if (!swd_read_dp(DP_IDCODE, &dp_id))
        {
            picoprobe_info("rp2040_swd_init_debug: diagnostic swd_read_dp(DP_IDCODE) FAILED\n");
        }
        else
        {
            picoprobe_info("rp2040_swd_init_debug: diagnostic DP_IDCODE=0x%08lx\n", (unsigned long)dp_id);
        }
        uint32_t dp_ctrl = 0;
        if (!swd_read_dp(DP_CTRL_STAT, &dp_ctrl))
        {
            picoprobe_info("rp2040_swd_init_debug: diagnostic swd_read_dp(DP_CTRL_STAT) FAILED\n");
        }
        else
        {
            picoprobe_info("rp2040_swd_init_debug: diagnostic DP_CTRL_STAT=0x%08lx\n", (unsigned long)dp_ctrl);
        }
    }
    ...
    do
    {
        ...
        phase = 1; /* dp_core_select */
        CHECK_ABORT(dp_core_select(core));

        phase = 2; /* swd_clear_errors */
        CHECK_ABORT(swd_clear_errors());

        phase = 3;                               /* DP_SELECT writes */
        ...
        CHECK_ABORT(swd_write_dp(DP_SELECT, 0));
        picoprobe_info("rp2040_swd_init_debug: success\n");
        return true;

    } while (--retries > 0);

    picoprobe_info("rp2040_swd_init_debug: failed at phase %d\n", phase);
    return false;
